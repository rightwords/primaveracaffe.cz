...
function fw_theme_font_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Lato font: on or off', 'coffeeking' ) ) {
					...
					<div class="contributor-summary">
						...
						<a class="button contributor-posts-link"
						   href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>">
							<?php printf( _n( '%d Article', '%d Articles', $post_count, 'coffeeking' ), $post_count ); ?>
						</a>
					</div>
					<!-- .contributor-summary -->
				</div>
...
if ( ! function_exists( 'fw_theme_paging_nav' ) ) : /**
	...
	function fw_theme_paging_nav( $wp_query = null ) {
		...
		$links = paginate_links( array(
			...
			'prev_text' => esc_html__( 'Prev', 'coffeeking' ),
			'next_text' => esc_html__( 'Next', 'coffeeking' ),
		) );
		...
		<nav class="navigation paging-navigation clearfix" role="navigation">
			<h1 class="screen-reader-text"><?php echo esc_html__( 'Posts navigation', 'coffeeking' ); ?></h1>
			<div class="pagination loop-pagination">
				<?php if ( $paged === 1 ) { echo '<a href="#" class="prev page-numbers disabled">' . esc_html__( 'Prev', 'coffeeking' ) . '</a>';} ?>
				<?php echo wp_kses_post( $links ); ?>
				<?php if ( $paged === $wp_query->max_num_pages ) { echo '<a href="#" class="next page-numbers disabled">' . esc_html__( 'Next', 'coffeeking' ) . '</a>';} ?>
			</div>
			<!-- .pagination -->
				</nav><!-- .navigation -->
			<?php
...
if ( ! function_exists( 'fw_theme_post_nav' ) ) : /**
	...
	function fw_theme_post_nav() {
		...
		<nav class="navigation post-navigation clearfix" role="navigation">
			<h1 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'coffeeking' ); ?></h1>

			<div class="nav-links">
				<?php
				if ( is_attachment() ) :
					previous_post_link( '%link', wp_kses_post( '<span class="meta-nav">Published In</span>%title', 'coffeeking' ) );
				else :
					previous_post_link( '%link', wp_kses_post( '<span class="meta-nav">&laquo;</span> %title', 'coffeeking' ) );
					next_post_link( '%link', wp_kses_post( '%title <span class="meta-nav">&raquo;</span>', 'coffeeking' ) );
				endif;
				?>
				</div>
				<!-- .nav-links -->
...
if ( ! function_exists( 'fw_theme_posted_on' ) ) : /**
	...
	function fw_theme_posted_on() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="featured-post">' . esc_html__( 'Sticky', 'coffeeking' ) . '</span>';
		}
